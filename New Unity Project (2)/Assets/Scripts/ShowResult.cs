...
public class ShowResult : MonoBehaviour
{
    ...
    void Start()
    {
        b = true;
        time = 3f;
        bgg = this.gameObject.AddComponent<AudioSource>();
        bgg.volume = 0.8f;
        bgg.clip = bg;
        if (record.A <= 0)
        {
            AudioSource.PlayClipAtPoint(lose, transform.localPosition);
            r.text = "Failed!";
            GetComponent<Text>().text = "維生素D：蛋黃、木耳、陽光\n"
        ...
        else if (record.B <= 0)
        {
            AudioSource.PlayClipAtPoint(lose, transform.localPosition);
            r.text = "Failed!";
            GetComponent<Text>().text = "維生素E：植物油\n"
        ...
        else if (record.C <= 0)
        {
            AudioSource.PlayClipAtPoint(lose, transform.localPosition);
            r.text = "Failed!";
            GetComponent<Text>().text = "鉀：水果、海帶、菇類\n"
        ...
        else if (record.D <= 0)
        {
            AudioSource.PlayClipAtPoint(lose, transform.localPosition);
            r.text = "Failed!";
            GetComponent<Text>().text = "鈣：牛奶、蔬菜、豆腐\n"
        ...
        else if (record.E <= 0)
        {
            AudioSource.PlayClipAtPoint(lose, transform.localPosition);
            r.text = "Failed!";
            GetComponent<Text>().text = "鎂：核桃、薏仁、綠色蔬菜\n"
        ...
        else if (record.F <= 0)
        {
            AudioSource.PlayClipAtPoint(lose, transform.localPosition);
            r.text = "Failed!";
            GetComponent<Text>().text = "鐵：鴨血、牛肉、魚肉\n"
        ...
        else if (record.G <= 0)
        {
            AudioSource.PlayClipAtPoint(lose, transform.localPosition);
            r.text = "Failed!";
            GetComponent<Text>().text = "鋅：堅果、生蠔、肉\n"
        ...
        else if (record.H <= 0)
        {
            AudioSource.PlayClipAtPoint(lose, transform.localPosition);
            r.text = "Failed!";
            GetComponent<Text>().text = "碘：海菜、牡蠣\n"
                                        + "為與生長發育有關的甲狀腺素成分之一，可調節體內基礎代謝。缺乏時，會影響甲狀腺功能，體能降低、甲狀腺腫大，婦女懷孕前更需特別注意攝取以免阻礙胎兒腦部發育。";
        }
        else
        {
            AudioSource.PlayClipAtPoint(win, transform.localPosition);
            r.text = "Congrats!";
            GetComponent<Text>().text = "點擊以下按鈕來獲得更多的資訊ㄛ !";
        }
    }
